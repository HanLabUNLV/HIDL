...
if((length(tree$tip.label)>10)){
     ...
     #setwd("PseKRAAC.g1_mv_results") #changed for testing purposes, orig: /data1/home/dbarth/LinkerProject/ModelLengths/
     #sink(file=paste('mv_model_fit',gt,paste(linker,'.txt',sep=''),sep='_'),append=FALSE)

     #BM model
     print(cor(otd[varnames]))
     ...
     #WN model
     wndata = as.matrix(data[varnames])
