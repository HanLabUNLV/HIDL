...
masterdf = pd.DataFrame(columns=['aliphatic','aromatic','positive_charge','negative_charge','neutral_charge', 'pid','treeID','linker','domain','length'])
...
def process_file(ff):
    if 'Linker' in ff:
            ...
            df.columns = ['aliphatic','aromatic','positive_charge','negative_charge','neutral_charge']
...
#add all linkers
linker_files = [x for x in ls(linker_feature_dir) if 'GAAC' in x]
#add all domains
domain_files = [x for x in ls(domain_feature_dir) if 'GAAC' in x]
#extend 
...
masterdf.to_csv('total_GAAC_data.tsv',sep='\t',index=False)
